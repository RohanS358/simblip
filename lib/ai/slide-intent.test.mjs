...
test('a sub-section becomes its own slide instead of a "(continued)" run', () => {
  // Measured on a real "slides + simulation for a simple pendulum" answer:
  // the model used four heading levels (#, ##, ###, ####). Only #/## split
  // slides, so its four "#### Step N:" sub-sections were crushed into one
  // body and split by LINE COUNT — producing four consecutive slides all
  // titled "Derivation (continued)", where the natural break was already
  // written in the source.
  const md = [
    '## Derivation',
    '### Step 1: Analyze Forces',
    'Resolve the weight along the arc.',
    '### Step 2: Small Angles',
    'Take sin θ ≈ θ.',
    '### Step 3: Newton',
    'Apply F = ma along the arc.',
  ].join('\n')
  const slides = answerToSlides(md, 'q')
  const headings = slides.slice(1).map((s) => textOf(s[0]))
  assert.ok(headings.some((h) => /Step 1/.test(h)), `no slide for Step 1: ${JSON.stringify(headings)}`)
  assert.ok(headings.some((h) => /Step 2/.test(h)), `no slide for Step 2: ${JSON.stringify(headings)}`)
  assert.ok(headings.some((h) => /Step 3/.test(h)), `no slide for Step 3: ${JSON.stringify(headings)}`)
  // and every step's body followed its own heading, not someone else's
  assert.match(allText(slides), /Resolve the weight/)
  assert.match(allText(slides), /Apply F = ma/)
})
