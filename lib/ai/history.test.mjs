...
test('routing classifies the new prompt, not the transcript', () => {
  ...
  // The raw prompt, not `userPrompt` (which now carries the page digest and
  // the folded transcript). A second argument is fine — that is the
  // has-context flag the edit lane gates on.
  ...
  assert.doesNotMatch(routeSrc, /classifyIntent\(userPrompt/,
    'classifying the folded prompt would let page content or history pick the lane')
})
