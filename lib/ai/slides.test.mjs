...
const { answerToSlides, blocksToSlides, parse } = await import(bundle)

// Slide text is stored as a SERIALIZED {text, marks} blob (lib/text/marks.ts)
// once it carries any formatting — body copy now carries an explicit size
// mark so it is legible from across a room, so reading .value raw would
// return JSON rather than the words.
const text = (obj) => {
  const raw = obj.parameters.text?.kind === 'string' ? obj.parameters.text.value : ''
  return raw ? parse(raw).text : ''
}
...
test('a heading starts a new slide, in order', () => {
  ...
  assert.match(text(slides[1][0]), /Given\b/)
  assert.match(text(slides[2][0]), /Derivation\b/)
})
...
test('prose before the first heading is folded into a slide, not dropped', () => {
  ...
  // The trailing colon is stripped: "### Given:" reads as an unfinished
  // sentence once the line is a slide TITLE rather than a run-in heading.
  assert.match(allText, /Result\b/)
})
