...
// A non-finite bound must never reach the store as a coordinate.
//
// viewportBounds() divides by the page's zoom, so a viewport restored with
// zoom 0/undefined hands this module a NaN edge. Math.max(NaN, ...) is NaN,
// and NaN serialises to `null` in the store — which renders at left:0 AND
// passes every bounds check (`null > right` is false), so the object sits
// off-frame with nothing reporting it. Observed on a real AI pendulum whose
// slider arrived with position.x === null.
test('a non-finite bound never yields a non-finite coordinate', () => {
  const size = { w: 200, h: 100 }
  for (const bad of [
    { left: NaN, top: 0, right: 960, bottom: 540 },
    { left: 0, top: NaN, right: 960, bottom: 540 },
    { left: 0, top: 0, right: NaN, bottom: 540 },
    { left: 0, top: 0, right: 960, bottom: Infinity },
    { left: NaN, top: NaN, right: NaN, bottom: NaN },
  ]) {
    const got = clampToBounds({ x: 60, y: 460 }, size, bad)
    assert.ok(Number.isFinite(got.x), `x finite for ${JSON.stringify(bad)}, got ${got.x}`)
    assert.ok(Number.isFinite(got.y), `y finite for ${JSON.stringify(bad)}, got ${got.y}`)
  }
})

test('placeAnswerAndScene survives a NaN frame without emitting null', () => {
  const bad = { left: NaN, top: 0, right: NaN, bottom: 540 }
  const got = placeAnswerAndScene({ w: 520, h: 300 }, true, bad)
  for (const [k, p] of Object.entries(got)) {
    assert.ok(Number.isFinite(p.x), `${k}.x finite, got ${p.x}`)
    assert.ok(Number.isFinite(p.y), `${k}.y finite, got ${p.y}`)
  }
})
