...
for (const [name, src] of Object.entries(CIRCUITS)) {
  test(`${name}: no two nets share a lane`, () => {
    // Two DIFFERENT nets drawn along the same stretch render as one line, so a
    // connection silently disappears from the sheet — invisible to a crossing
    // count, which only sees proper X-intersections. Branches of the SAME net
    // may share a lane: that is one conductor, drawn once.
    const objs = run(src)
    const ws = wires(objs)
    const pts = ws.map(worldPts)

    // Union wires of the same net. Sharing a vertex is not enough: a fan-out
    // branch leaves its trunk partway ALONG a segment, and two wires driven by
    // the same pin are one net even where their drawn paths only meet at that
    // pin. Group by both — a shared point anywhere on either polyline.
    const parent = ws.map((_, i) => i)
    const find = (x) => { while (parent[x] !== x) x = parent[x] = parent[parent[x]]; return x }
    const onPath = (p, path) => path.slice(1).some((b, k) => {
      const a = path[k]
      const cross = (p.x - a.x) * (b.y - a.y) - (p.y - a.y) * (b.x - a.x)
      if (Math.abs(cross) > 0.01 * Math.max(1, Math.hypot(b.x - a.x, b.y - a.y))) return false
      const dot = (p.x - a.x) * (b.x - a.x) + (p.y - a.y) * (b.y - a.y)
      return dot >= -0.01 && dot <= (b.x - a.x) ** 2 + (b.y - a.y) ** 2 + 0.01
    })
    for (let i = 0; i < ws.length; i++)
      for (let j = i + 1; j < ws.length; j++) {
        const touch =
          pts[i].some((a) => pts[j].some((b) => Math.hypot(a.x - b.x, a.y - b.y) < 0.5)) ||
          pts[i].some((a) => onPath(a, pts[j])) ||
          pts[j].some((b) => onPath(b, pts[i]))
        if (touch) {
    ...
    const segs = pts.map((p) => p.slice(1).map((b, i) => [p[i], b]))
    for (let i = 0; i < segs.length; i++)
      for (let j = i + 1; j < segs.length; j++) {
        if (find(i) === find(j)) continue
        for (const [a1, a2] of segs[i])
          for (const [b1, b2] of segs[j]) {
            const horiz = Math.abs(a1.y - a2.y) < 0.5 && Math.abs(b1.y - b2.y) < 0.5 && Math.abs(a1.y - b1.y) < 0.5
            const vert = Math.abs(a1.x - a2.x) < 0.5 && Math.abs(b1.x - b2.x) < 0.5 && Math.abs(a1.x - b1.x) < 0.5
            const run_ = horiz
              ...
              : vert
                ...
                : -1
            assert.ok(run_ <= 1,
              `two different nets share ${run_.toFixed(0)}px of lane — one of them is invisible`)
          }
      }
  })
}
