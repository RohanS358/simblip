...
// ── 'pair' probes: a control's single target spans THREE params ─────────────
// Mirrors probeLinks / writeProbeLinks in lib/scene/probes.ts for mode 'pair'.
// The round trip is the whole contract: what the layer writes on drop is what
// the Inspector reads back, and clearing returns the control to unbound.
{
  const spec = {
    param: 'targetObjectId',
    nameParam: 'targetParamName',
    typeParam: 'targetType',
    mode: 'pair',
  }
  const str = (v) => ({ kind: 'string', value: v })
  const get = (params, name) =>
    params[name]?.kind === 'string' ? params[name].value : ''

  const probeLinks = (params) =>
    get(params, spec.typeParam) !== 'objectParam' || !get(params, spec.param)
      ? []
      : [{ index: 0, objectId: get(params, spec.param), channel: get(params, spec.nameParam) }]

  const writeProbeLinks = (links) => {
    const l = links[0]
    return {
      [spec.typeParam]: str(l ? 'objectParam' : 'variable'),
      [spec.param]: str(l?.objectId ?? ''),
      [spec.nameParam]: str(l?.channel ?? ''),
    }
  }

  // A control pointed at a page variable draws no arrow — there is no object.
  assert.deepEqual(
    probeLinks({ targetType: str('variable'), targetParamName: str('gravity') }),
    [],
    'variable target has no arrow'
  )

  const bound = writeProbeLinks([{ index: 0, objectId: 'obj-7', channel: 'value' }])
  assert.deepEqual(
    probeLinks(bound),
    [{ index: 0, objectId: 'obj-7', channel: 'value' }],
    'drop → read back the same binding'
  )
  assert.equal(bound.targetType.value, 'objectParam', 'drop switches to objectParam mode')

  // A second drop REPLACES (a control has one target), and removing unbinds.
  const rebound = writeProbeLinks([{ index: 0, objectId: 'obj-9', channel: 'x' }])
  assert.deepEqual(probeLinks(rebound), [{ index: 0, objectId: 'obj-9', channel: 'x' }], 'replaces')
  const cleared = writeProbeLinks([])
  assert.deepEqual(probeLinks(cleared), [], 'remove unbinds')
  assert.equal(cleared.targetType.value, 'variable', 'unbound falls back to variable mode')
}

console.log('probes: pair-mode checks passed')
