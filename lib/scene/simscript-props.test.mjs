...
// ── Control wiring ─────────────────────────────────────────────────────────
// A scripted slider used to drive nothing at all. Two independent bugs: the
// generic prop loop stringifies every value, so passing a create() handle as
// targetObjectId stored the literal "[object Object]"; and targetType kept
// the factory's "variable" default, so getTargetValue (lib/scene/
// control-targets.ts) looked up a page variable and ignored the object.

/** The parameter set createGeometry seeds a slider with. */
const sliderParams = () => ({
  targetType: { kind: 'string', value: 'variable' },
  targetObjectId: { kind: 'string', value: '' },
  targetParamName: { kind: 'string', value: '' },
  min: { kind: 'number', expr: '0', value: 0 },
  max: { kind: 'number', expr: '100', value: 100 },
  value: { kind: 'number', expr: '0', value: 0 },
  label: { kind: 'string', value: '' },
})

test('a slider given a create() handle binds to that object', () => {
  const o = obj(sliderParams(), { kind: 'slider' })
  // What a script passes: the handle returned by create(), not a raw id.
  applyKindProps(o, 'slider', { targetParamName: 'mass', targetObjectId: { id: 'block-7' } })
  assert.equal(o.parameters.targetObjectId.value, 'block-7',
    'a handle must resolve to its id, not stringify to "[object Object]"')
  assert.equal(o.parameters.targetType.value, 'objectParam',
    'without objectParam the control reads a page variable and drives nothing')
  assert.equal(o.parameters.targetParamName.value, 'mass')
})

test('a plain string id works too', () => {
  const o = obj(sliderParams(), { kind: 'slider' })
  applyKindProps(o, 'slider', { targetObjectId: 'block-7', targetParamName: 'k' })
  assert.equal(o.parameters.targetObjectId.value, 'block-7')
  assert.equal(o.parameters.targetType.value, 'objectParam')
})

test('a slider with no target stays a variable control', () => {
  const o = obj(sliderParams(), { kind: 'slider' })
  applyKindProps(o, 'slider', { min: 1, max: 50, label: 'Gravity', targetParamName: 'g' })
  assert.equal(o.parameters.targetType.value, 'variable',
    'a variable-driving slider is a legitimate scene — do not force objectParam')
  assert.equal(o.parameters.label.value, 'Gravity')
})

test('an explicit targetType is respected over the inferred one', () => {
  const o = obj(sliderParams(), { kind: 'slider' })
  applyKindProps(o, 'slider', { targetObjectId: 'x1', targetType: 'variable' })
  assert.equal(o.parameters.targetType.value, 'variable')
})

test('buttons and triggers bind the same way', () => {
  for (const kind of ['button', 'trigger']) {
    const o = obj(sliderParams(), { kind })
    applyKindProps(o, kind, { targetObjectId: { id: 'b2' }, targetParamName: 'speed' })
    assert.equal(o.parameters.targetObjectId.value, 'b2', `${kind} must bind by id`)
    assert.equal(o.parameters.targetType.value, 'objectParam')
  }
})
