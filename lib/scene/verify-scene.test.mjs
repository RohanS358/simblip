...
test('a rod pivoted on a hinge is attached, not dangling', () => {
  // A hinge is not a rigid body, so buildWorld's point query finds nothing
  // under the endpoint resting on it and Matter treats the null bodyA as a
  // WORLD anchor — which is exactly what a fixed pivot is. Checking against
  // rigid/static bodies alone reported "not attached at its start" for every
  // hinge-pivoted pendulum, the most common mechanics scene there is, while
  // the scene swung perfectly.
  const scene = [
    mk('pivot', 'circle', { pos: { x: 189, y: 89 }, size: { w: 22, h: 22 }, behaviors: ['hinge'] }),
    mk('bob', 'circle', { pos: { x: 165, y: 305 }, size: { w: 70, h: 70 }, behaviors: ['rigidBody'] }),
    mk('rod', 'line', {
      pos: { x: 200, y: 100 }, size: { w: 2, h: 240 },
      points: [[0, 0], [0, 240]], behaviors: ['rod'],
    }),
  ]
  const errors = verifyScene(scene).issues.filter((i) => i.level === 'error')
