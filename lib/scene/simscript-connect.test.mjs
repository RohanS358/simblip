...
test('a voltmeter across a resistor keeps both leads on their own terminals', () => {
  // The reported bug, and the reason every scripted voltmeter read 0 V.
  //
  // The re-router looked its wire up by the COMPONENT PAIR alone. A voltmeter
  // measuring one resistor is two wires between the same pair, so both edges
  // found the same wire: it was routed twice (the second edge winning) and the
  // other wire was left where the pre-layout draw had put it, far from any
  // terminal once the layout moved the parts. lib/circuit/engine.ts builds
  // nets from coincident world points, so that lead sat on a net of its own
  // and the meter measured nothing.
  const { objs } = run(`
    var bat = create("battery", { V: 9 });
    var r1 = create("resistor", { R: 330 });
    var vm = create("voltmeter", {});
    connect(bat.positive, r1.a, "wire");
    connect(r1.b, bat.negative, "wire");
    connect(vm.a, r1.a, "wire");
    connect(vm.b, r1.b, "wire");
  `)
  assert.deepEqual(wiredPins(objs, 'voltmeter'), [0, 1],
    'the meter must land one lead on each of its own terminals')
  // Both of the resistor's terminals carry a wire from the supply loop AND a
  // meter lead: four endpoints, two per terminal.
  assert.deepEqual(wiredPins(objs, 'resistor'), [0, 0, 1, 1],
    'each resistor terminal takes a supply wire and a meter lead')
})

test('two meters on one divider do not stack on the same spot', () => {
  // Both probes were placed on one fixed line above the loop, centred on the
  // part they measure. The top and bottom rows of the drawn rectangle share a
  // column, so two meters reading the two halves of a divider landed on
  // exactly the same point — and coincident terminals are ONE net to
  // lib/circuit/engine.ts, which shorted the divider out entirely.
  const { objs } = run(`
    var bat = create("battery", { V: 9 });
    var r1 = create("resistor", { R: 220 });
    var r2 = create("resistor", { R: 330 });
    var v1 = create("voltmeter", {});
    var v2 = create("voltmeter", {});
    connect(bat.positive, r1.a, "wire");
    connect(r1.b, r2.a, "wire");
    connect(r2.b, bat.negative, "wire");
    connect(v1.a, r1.a, "wire");
    connect(v1.b, r1.b, "wire");
    connect(v2.a, r2.a, "wire");
    connect(v2.b, r2.b, "wire");
  `)
  const meters = objs.filter((o) => o.geometry.symbol === 'voltmeter')
  assert.equal(meters.length, 2)
  const pts = meters.map((m) => terminalsOf(m).map((t) => terminalWorld(m, t)))
  for (const a of pts[0]) {
    for (const b of pts[1]) {
      assert.ok(Math.hypot(a.x - b.x, a.y - b.y) > 14,
        'meter terminals within snap distance are one net — the meters short the circuit')
    }
  }
})
