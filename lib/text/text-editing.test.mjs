...
// ---------------------------------------------------------------------------
// checkLineSwitch must never collapse a live range selection.
//
// A mouse drag-selection ENDS with a click event, and Shift+Arrow ends with a
// keyup — both call checkLineSwitch. It used to unconditionally run
// activateLine(), whose placeCaretAt() does sel.removeAllRanges(): the range
// the user just dragged was thrown away and replaced with a bare caret.
//
// The damage showed up one step later, in the panel. With the selection
// collapsed, setSpan reads start === end and takes the "a collapsed caret
// means apply to the WHOLE BOX" branch — so picking a size or colour right
// after selecting two words restyled every character in the text box. That is
// the reported symptom: "I select a small portion, and the edit applies to a
// lot more."
//
// The rule is pure, so it's modelled here the same way resolveSpan is:
// a range selection switches the active line for RENDERING only and leaves
// the caret (and therefore the selection) untouched.

/** Mirrors checkLineSwitch's decision (components/objects/text.tsx). */
const lineSwitch = ({ span, caret, active }) => {
  if (span) {
    return span.s.line === active
      ? { active, movesCaret: false }
      : { active: span.s.line, movesCaret: false }
  }
  if (!caret || caret.line === active) return { active, movesCaret: false }
  return { active: caret.line, movesCaret: true }
}

test('selection: a drag-selection ending on another line never moves the caret', () => {
  // Dragged from line 2 into line 3; activeRef still points at line 0.
  const span = { s: { line: 2, offset: 4 }, e: { line: 3, offset: 9 } }
  const r = lineSwitch({ span, caret: null, active: 0 })
  assert.equal(r.active, 2, 'the active line follows the selection start')
  assert.equal(
    r.movesCaret,
    false,
    'moving the caret here would call removeAllRanges and destroy the selection'
  )
})

test('selection: a single-line range on an inactive line keeps its range', () => {
  const span = { s: { line: 1, offset: 2 }, e: { line: 1, offset: 7 } }
  const r = lineSwitch({ span, caret: null, active: 0 })
  assert.equal(r.active, 1)
  assert.equal(r.movesCaret, false, 'a range must survive the line switch intact')
})

test('selection: a plain caret still activates and moves as before', () => {
  const r = lineSwitch({ span: null, caret: { line: 2, offset: 3 }, active: 0 })
  assert.equal(r.active, 2)
  assert.equal(r.movesCaret, true, 'with no range there is nothing to preserve')
})

test('selection: a surviving range keeps setSpan off the whole-box path', () => {
  // The end-to-end consequence: span survives, so start !== end, so setSpan
  // marks only the selected characters instead of applyWholeBox().
  const text = 'hello world'
  const span = { s: { line: 0, offset: 0 }, e: { line: 0, offset: 5 } }
  assert.equal(lineSwitch({ span, caret: null, active: 1 }).movesCaret, false)

  const start = span.s.offset
  const end = span.e.offset
  assert.notEqual(start, end, 'a live range must not read as a collapsed caret')
  const marks = applyMark([], start, end, 'size', '32')
  const size = marks.find((m) => m.kind === 'size')
  assert.deepEqual(
    [size.start, size.end],
    [0, 5],
    'only "hello" is resized — not all of "' + text + '"'
  )
})
