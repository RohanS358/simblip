...
// ── Global Preference & Folder Cascading Gate Tests ──────────────────────────

const ancestorFolderSynced = (nodes, nodeId) => {
  let cur = nodes[nodeId]
  while (cur) {
    const parent = cur.parentId ? nodes[cur.parentId] : undefined
    if (parent?.kind === 'folder' && parent.syncEnabled === true) return true
    cur = parent
  }
  return false
}

const syncedPagesWithHierarchy = (nodes, globalPrefAllowsPage = () => false) => {
...
test('folder gate: page inside synced folder inherits syncEnabled', () => {
  const nodes = {
    f1: { id: 'f1', kind: 'folder', syncEnabled: true, parentId: null },
    p1: { id: 'p1', kind: 'page', pageKind: 'pdf', parentId: 'f1' },
    p2: { id: 'p2', kind: 'page', pageKind: 'board', parentId: null },
  }
  const res = syncedPagesWithHierarchy(nodes)
  assert.deepEqual(res.map((n) => n.id), ['p1'])
})

test('folder gate: page inside nested folder inherits sync from root notebook', () => {
  const nodes = {
    root: { id: 'root', kind: 'folder', syncEnabled: true, parentId: null },
    sub: { id: 'sub', kind: 'folder', parentId: 'root' },
    p1: { id: 'p1', kind: 'page', pageKind: 'doc', parentId: 'sub' },
  }
  const res = syncedPagesWithHierarchy(nodes)
  assert.deepEqual(res.map((n) => n.id), ['p1'])
})

test('global category gate: matching pageKind syncs when global category enabled', () => {
  const nodes = {
    p1: { id: 'p1', kind: 'page', pageKind: 'pdf', parentId: null },
    p2: { id: 'p2', kind: 'page', pageKind: 'board', parentId: null },
  }
  // Global rule: documents enabled (pdf -> documents)
  const allowsDocOnly = (kind) => kind === 'pdf'
  const res = syncedPagesWithHierarchy(nodes, allowsDocOnly)
  assert.deepEqual(res.map((n) => n.id), ['p1'])
})
